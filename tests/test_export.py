...
from vespatune import VespaTune, VespaTuneExport, export_model
...
def trained_binary_model_for_export(binary_classification_data):
    ...
    valid_df = pd.read_csv(data["valid_path"])
...
class TestONNXExport:
    ...
    def test_export_binary_model(self, trained_binary_model_for_export):
        ...
        output_dir = os.path.join(
            trained_binary_model_for_export["temp_dir"], "onnx_output"
        )
    ...
    def test_export_regression_model(self, trained_regression_model_for_export):
        ...
        exporter = VespaTuneExport(
            model_path=trained_regression_model_for_export["model_path"]
        )

        output_dir = os.path.join(
            trained_regression_model_for_export["temp_dir"], "onnx_output"
        )
        exported_files = exporter.export_to_onnx(output_dir=output_dir)
    ...
    def test_export_multi_target_model(self, trained_multi_target_model_for_export):
        ...
        exporter = VespaTuneExport(
            model_path=trained_multi_target_model_for_export["model_path"]
        )

        output_dir = os.path.join(
            trained_multi_target_model_for_export["temp_dir"], "onnx_output"
        )
        exported_files = exporter.export_to_onnx(output_dir=output_dir)

        # Should export one model per target
        assert len(exported_files) == len(
            trained_multi_target_model_for_export["targets"]
        )
    ...
    def test_export_default_output_dir(self, trained_binary_model_for_export):
        ...
        expected_dir = os.path.join(
            trained_binary_model_for_export["model_path"], "onnx"
        )
