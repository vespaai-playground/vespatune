...
# Usage
Training a model using VespaTune is a piece of cake. All you need is some tabular data.
...
Train the model using the `vespatune train` command. The parameters are same as above.
...
You can also serve the trained model using the `vespatune serve` command.

```bash
vespatune serve --model_path outputs/mll --host 0.0.0.0 --debug
```

To know more about a command, run:

    `vespatune <command> --help` 

```
vespatune train --help


usage: vespatune <command> [<args>] train [-h] --train_filename TRAIN_FILENAME [--test_filename TEST_FILENAME] --output
